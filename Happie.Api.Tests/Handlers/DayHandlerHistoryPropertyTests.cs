    ...
    public Property UpsertDishAsync_NothingChanged_NoHistoryEntry()
    {
        return Prop.ForAll(
            ...
            async (description, dinnerTime) =>
            {
                ...
                var existingDish = new DishRecord(householdId, date, description, Guid.NewGuid(), DateTimeOffset.UtcNow, dinnerTime);
    ...
    public Property UpsertDishAsync_OnlyDescriptionChanged_DishHistoryEntry()
    {
        return Prop.ForAll(
            ...
            async (descriptions, dinnerTime) =>
            {
                ...
                var existingDish = new DishRecord(householdId, date, oldDescription, Guid.NewGuid(), DateTimeOffset.UtcNow, dinnerTime);
                var (sut, capturedEntries) = CreateSutWithDishCapture(existingDish);
    ...
    public Property UpsertDishAsync_OnlyDinnerTimeSet_DinnerTimeHistoryEntry()
    {
        return Prop.ForAll(
            ...
            async (description, times) =>
            {
                ...
                var existingDish = new DishRecord(householdId, date, description, Guid.NewGuid(), DateTimeOffset.UtcNow, oldDinnerTime);
                var (sut, capturedEntries) = CreateSutWithDishCapture(existingDish);
    ...
    public Property UpsertDishAsync_BothChangedDinnerTimeSet_DishAndDinnerTimeHistoryEntry()
    {
        return Prop.ForAll(
            ...
            async (descriptions, times) =>
            {
                ...
                var existingDish = new DishRecord(householdId, date, oldDescription, Guid.NewGuid(), DateTimeOffset.UtcNow, oldDinnerTime);
                var (sut, capturedEntries) = CreateSutWithDishCapture(existingDish);
    ...
    public Property UpsertDishAsync_AnyCombination_AtMostOneHistoryEntry()
    {
        return Prop.ForAll(
            ...
            async input =>
            {
                ...
                var existingDish = new DishRecord(householdId, date, input.OldDescription, Guid.NewGuid(), DateTimeOffset.UtcNow, input.OldDinnerTime);
                var (sut, capturedEntries) = CreateSutWithDishCapture(existingDish);
